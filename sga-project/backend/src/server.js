       ...
       WHERE am.id = $1`,
		...
		if (movementResult.rows.length === 0) {
			return res
				...
				.json({
					message:
						'Movimentação não encontrada para nomear o arquivo.',
				});
		}
	...
	async (req, res) => {
		...
		if (!username || !email || !full_name || !role) {
			return res
				...
				.json({
					message:
						'Campos obrigatórios: Username, Email, Nome e Perfil.',
				});
		}
	...
	try {
		...
		if (itemTypes.length === 0) {
			return res
				...
				.json({
					message:
						'Nenhum tipo de item encontrado para gerar relatório.',
				});
		}
		...
		try {
			...
			if (oldItemTypeResult.rows.length === 0) {
				return res
					...
					.json({
						message:
							'Tipo de item não encontrado para atualização.',
					});
			}
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`, // <--- Adicionado $9
			...
			if (error.code === '23505') {
				...
				return res
					...
					.json({
						message:
							'Já existe uma unidade com este nome ou código.',
					});
			}
			res.status(500).json({
	...
	async (req, res) => {
		...
		if (!Array.isArray(assetIds) || assetIds.length === 0) {
			return res
				...
				.json({
					message:
						'É necessário fornecer uma lista de IDs de ativos.',
				});
		}
		if (!reason) {
			return res
	...
	async (req, res) => {
		...
		if (!req.file)
			return res
				...
				.json({
					message:
						'O arquivo de evidência (laudo/B.O.) é obrigatório.',
				});
		if (!reason) {
...
app.post('/api/assets/find-for-return', authenticateToken, async (req, res) => {
	...
	if (!identifier) {
		return res
			...
			.json({
				message:
					'Identificador (patrimônio ou nº de série) é obrigatório.',
			});
	}
    ...
    `;
		...
		if (personResult.rows.length === 0) {
			return res
				...
				.json({
					message:
						'Cadastro do responsável pelo ativo não encontrado.',
				});
		}
		...
		try {
			...
			if (!filePath) {
				return res
					...
					.json({
						message:
							'Nenhum recibo assinado foi anexado a este lote.',
					});
			}
