...
app.delete('/api/delivery-batches/:id', authenticateToken, authorizePermission('MENU_ESCOLAR'), async (req, res) => {
  ...
  try {
    ...
    await client.query('COMMIT');
    await logAudit(req.user.id, 'delete_delivery_batch', 'delivery_batches', batch_id, { name: batch.name, status: batch.status }, ipAddress);
    ...
    if (error.code === '23503') {
        return res.status(400).json({ message: 'Não é possível excluir este lote pois existem registros dependentes.' });
    }
    ...
    try {
        ...
        try {
            ...
            const rpaData = rpaRes.rows.map(r => ({
                ...
                pendentes: Math.max(0, parseInt(r.total, 10) - parseInt(r.delivered, 10)),
            ...
            res.json({
                ...
                weeklyVelocity,
                projectedDate: projectedDate ? projectedDate.toISOString() : null, // <<< AQUI OCORRIA O ERRO! AGORA CORRIGIDO.
